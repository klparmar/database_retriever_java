...
public class A02FrontEnd extends JFrame implements ItemListener, ActionListener{
    ...
    A02MiddleTier proc = new A02MiddleTier(); //proccessor for the queries
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if(allDates.isSelected()) {
    		if(eventConference.isSelected()) {
    			input +=proc.q1 + "\n"; 
    			output += proc.printResult(proc.q1 + ";");
    			conf = 1;
    		}
    		if(eventJournal.isSelected()) {
    			if(conf == 1)
    				input +="UNION \n";
    			input +=proc.q2 + "\n";
    			output += proc.printResult(proc.q2 + ";");
    			jour = 1;
    		}
    		if(eventBook.isSelected()) {
    			if(jour == 1 || conf == 1)
    				input +="UNION \n";
    			input +=proc.q3 + "\n";
    			output += proc.printResult(proc.q3 + ";");
    		}
    		
    		if(!eventConference.isSelected() && !eventJournal.isSelected() && !eventBook.isSelected()) { //if none are selected
    			input += proc.q1 + "\n" +"UNION \n"+ proc.q2 + "\n" +"UNION \n"+ proc.q3 + "\n";
    			output += proc.printResult(proc.q1 + ";") + proc.printResult(proc.q2 + ";") + proc.printResult(proc.q3 + ";");
    		}
    	}
    	
    	//if filter by dates WE ASSUME INPUT DATES ARE VALID
    	else if(dateRange.isSelected()) {
    		if(eventConference.isSelected()) {
    			input +=proc.q1 +" and evdate between '"+fromDate.getText()+"' AND '"+toDate.getText()+"'"+ "\n"; 
    			output += proc.printResult(proc.q1 + " and evdate between '"+fromDate.getText()+"' AND '"+toDate.getText()+"';");
    			conf = 1;
    		}
    		if(eventJournal.isSelected()) {
    			if(conf == 1)
    				input +="UNION \n";
    			input +=proc.q2 + " and activityDate between '"+fromDate.getText()+"' AND '"+toDate.getText()+"'"+ "\n"; 
    			output += proc.printResult(proc.q2 + " and ActivityDate between '"+fromDate.getText()+"' AND '"+toDate.getText()+"';");
    			jour = 1;
    		}
    		if(eventBook.isSelected()) {
    			if(jour == 1 || conf == 1)
    				input +="UNION \n";
    			input +=proc.q3 + " and activityDate between '"+fromDate.getText()+"' AND '"+toDate.getText()+"'"+ "\n"; 
    			output += proc.printResult(proc.q3 + " and ActivityDate between '"+fromDate.getText()+"' AND '"+toDate.getText()+"';");
    		}
